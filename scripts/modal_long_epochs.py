"""Three nine-epoch training jobs and fresh paired replays at epochs 5, 7 and 9."""
...
app = modal.App("patrick-long-epochs")
output = modal.Volume.from_name("janestreet-patrick-long-epochs", create_if_missing=True)
...
owners = modal.Dict.from_name("janestreet-long-epoch-calls", create_if_missing=True)
volumes = {
    "/study": output,
...
CONFIG = "configs/patrick_long_epochs.yaml"
CAPTURES = (5, 7, 9)
...
def train_seed(run_id, seed):
    ...
    try:
        ...
        def progress(record):
            ...
            check_history(saved["history"][:5], reference)
            if saved["epoch"] == 5 and saved["position"] == 0:
                original_weights = torch.load(
                    ORIGIN / "seeds" / str(seed) / "checkpoint/weights.pt",
                    map_location="cpu",
                    weights_only=True,
                )[0]
                if original_weights.keys() != saved["model"].keys() or any(
                    not torch.equal(value, original_weights[key])
            ...
            state = {
                ...
                **record,
                "seed": seed,
                "reference_epochs_verified": min(5, len(saved["history"])),
...
def coordinate(run_id, code_sha, commit):
    ...
    try:
        ...
        source = RestrictedDateSource(ParquetSource(raw), range(1380))
        ...
        old_key = read(ORIGIN / "preparation.json")["key"]
        old_features = read(Path("/original/preparation_cache") / old_key / "features.json")
        if prepared.features.state_dict() != old_features:
            raise ValueError("rebuilt preprocessing differs from epoch-five control")
        output.commit()
        ...
        write_json(
            ...
            {
                "passed": True,
                "preprocessing_matches_original": True,
        ...
        control = next(r for r in result["epochs"] if r["epoch"] == 5)
        import math

        for mode, trial, field in (
            ("offline", "frozen", "frozen_r2"),
            ("online", "lr_1e-4_persistent", "online_r2"),
        ):
            prior = read(ORIGIN / "trials" / trial / mode / "result.json")
            if (
                control["initial_weights_sha256"] != prior["initial_weights_sha256"]
                or control["scored_rows"] != prior["scored_rows"]
                or control["denominator"] != prior["denominator"]
                or not math.isclose(control[field], prior["score"], rel_tol=0, abs_tol=1e-10)
            ):
                raise ValueError("fresh epoch-five replay differs from original control")
        write_json(root / "control_verification.json", {"passed": True, "score_atol": 1e-10})
        tracker.log(
            {
